...
class Schedule extends CI_Controller
{
	...
	function index()
	{
		...
		foreach($this->months as $k=>$v)
		{
			$sql = "SELECT * 
					...
					WHERE DATE_FORMAT(ss.session_date,'%Y-%c') = '".$selected_year.'-'.$k."'";
	...
	{
		$programs = $this->db->get('tb_programtemplate')->result_array();
	...
	private function get_programs_setting($program_setting_id,$get_all = FALSE)
	{
		if($get_all)
		{
			return $this->db->order_by('name,title')->get('tb_programtemplate');
		}
		else 
		{
