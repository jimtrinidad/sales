...
$.fn.themeswitcher = function(settings){
    ...
    */
   switcherpane.mouseleave(function (){
       if(switcherpane.is(':visible')){$(this).spHide();}
   });
   
    // ako ang nag add nito para maging pataas ung pag display ng themes
    $.fn.riseUp = function()   {$(this).show("slide", {direction: "down"}, 400);}
    $.fn.riseDown = function() {$(this).hide("slide", {direction: "down"}, 100);}   

	//show/hide panel functions
	$.fn.spShow = function(){$(this).css({position: 'fixed',bottom: 1, left: button.offset().left}).riseUp();button.css(button_active);options.onOpen();}
	$.fn.spHide = function(){$(this).riseDown();button.css(button_default);options.onClose();}
	...
	switcherpane.find('a').click(function(){
		...
		$.cookie(options.cookieName, themeName,{path: '/',expires: 365});
		options.onSelect();
